...
class Auth extends CI_Controller {
	...
	public function recover()
	{
		if($this->session->userdata("islogin") === true){
			...
			$this->load->view('Auth/recover'/*, ['Levels'=> $levels = $this->Level->All()]*/);
	...
	public function resetting()
	{
		$this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email', 
			array('required' => 'Debes colocar una cuenta de correo para resetear la contraseña'));

		if($this->form_validation->run() == false){
			$this->load->view('Auth/recover'/*, ['Levels'=> $levels = $this->Level->All()]*/);
		}else{
			$usuario = $this->User->Check('email', $_POST['email'], '');
			if($usuario == NULL){
				echo "no existe";
				//$this->load->view('Auth/token', $msj = 'No existe el correo');
			}else{
				echo "existe";
			}	
		}
	...
	public function login()
	{
		if($this->session->userdata("islogin") === true){
			...
			if($_POST)
			{
				$usuario = $this->User->Check('account', $_POST['user'], $_POST['pass']);
				if($usuario){
					$usuario_data = array(
						'user'=>$usuario['user'],
